...
const BoxDetail = ({ boxId }) => {
  ...
  const ResourceMonitor = ({ boxId }) => {
    const { boxResources, fetchBoxResources, updateBoxResourceLimits } = useAppContext();
    ...
    const resources = boxResources[boxId] || {
      cpu: 0,
      memory: 0,
      networkRx: 0,
      networkTx: 0,
    ...
    useEffect(() => {
      // Initial fetch
      fetchBoxResources(boxId).catch(err => 
        console.error(`Error fetching box resources: ${err.message}`)
      );
      
      // Set up polling interval (every 5 seconds)
      const interval = setInterval(() => {
        fetchBoxResources(boxId).catch(err => 
          console.error(`Error fetching box resources in interval: ${err.message}`)
        );
      }, 5000);
      
      return () => clearInterval(interval);
    }, [boxId, fetchBoxResources]);
    
    // Update limits from resources when available
    useEffect(() => {
      if (resources && resources.resourceLimits) {
        setLimits({
          cpuLimit: resources.resourceLimits.cpuLimit || 80,
          memoryLimit: resources.resourceLimits.memoryLimit || 1024,
          networkLimit: resources.resourceLimits.networkLimit || 20
        });
      }
    }, [resources]);
    ...
    const handleSaveLimits = async () => {
      try {
        await updateBoxResourceLimits(boxId, limits);
      ...
      <div className="bg-gray-800 rounded-lg p-4 mt-4">
        <div className="flex justify-between items-center mb-4">
          ...
          <div className="flex space-x-2">
            <button
              onClick={() => fetchBoxResources(boxId)}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
          ...
          <div className="bg-gray-700 p-3 rounded-md">
            <div className="flex justify-between mb-1">
              ...
              <span>{(resources.networkRx + resources.networkTx).toFixed(2)} Mbps</span>
            </div>
            <div className="w-full bg-gray-600 rounded-full h-2.5">
              <div 
                className={`h-2.5 rounded-full ${
                  (resources.networkRx + resources.networkTx) > limits.networkLimit * 0.8 ? 'bg-red-600' : 
                  (resources.networkRx + resources.networkTx) > limits.networkLimit * 0.6 ? 'bg-yellow-500' : 'bg-blue-500'
                }`}
                style={{ width: `${Math.min(((resources.networkRx + resources.networkTx) / limits.networkLimit) * 100, 100)}%` }}
              ></div>
            </div>
            {isEditingLimits && (
              <div className="mt-2">
          ...
          <div className="bg-gray-700 p-3 rounded-md">
            <div className="flex justify-between mb-1">
              ...
              <span>{resources.diskUsage?.toFixed(2) || 0} GB</span>
            </div>
            <div className="w-full bg-gray-600 rounded-full h-2.5">
              <div 
                className="h-2.5 rounded-full bg-teal-500"
                style={{ width: `${Math.min((resources.diskUsage / 100) * 100, 100)}%` }}
              ></div>
            </div>
          </div>
          ...
          <div className="bg-gray-700 p-3 rounded-md">
            <div className="flex justify-between mb-1">
              ...
              <span>{resources.networkRx?.toFixed(2) || 0} Mbps</span>
            </div>
            <div className="w-full bg-gray-600 rounded-full h-2.5">
              <div 
          ...
          <div className="bg-gray-700 p-3 rounded-md">
            <div className="flex justify-between mb-1">
              ...
              <span>{resources.networkTx?.toFixed(2) || 0} Mbps</span>
            </div>
            <div className="w-full bg-gray-600 rounded-full h-2.5">
              <div 
        ...
        {(resources.cpu > limits.cpuLimit || 
          ...
          <div className="bg-red-900/30 border border-red-800 text-red-200 p-3 rounded-md mt-2">
            ...
            <p className="text-sm mt-1">
              ...
              take action to reduce resource usage if the condition persists.
            </p>
          </div>
        )}
      </div>
  ...
  return (
    <>
      ...
      {/* Add Resource Monitor if box is running */}
      {selectedBox && selectedBox.status === 'running' && (
        <ResourceMonitor boxId={selectedBox._id} />
      )}
    </>
  );
};
